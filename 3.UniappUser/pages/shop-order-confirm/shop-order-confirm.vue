					...
					success: async function(res) {
						if (res.confirm) {
							...
							for (let i = 0; i < _this.orderGoods.length; i++) {
                                ...
                                // 当前商品是否相同表
                                if(_this.orderGoods[i].tablename == `nongchanpin`){
                                     // 获取数量 
                                     let res = await _this.$api.info('nongchanpin', _this.orderGoods[i].goodid);
                                     // 单次限次
                                        ...
                                        break;
                                         ...
                                         if(_this.seat!=1){
                                            res.data.alllimittimes = res.data.alllimittimes - _this.orderGoods[i].buynumber;
                                             // 库存充足则减少库存数量
                                             await _this.$api.update(`nongchanpin`,  res.data);
                                         }else{
